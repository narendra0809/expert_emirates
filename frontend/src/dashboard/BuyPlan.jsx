...
export default function BuyPlan() {
  ...
  const fetchCurrentPlans = async () => {
    try {
      ...
      if (res.status !== 200) {
        throw new Error("Unable to fetch current plans !");
      }
      const filteredPlans = res.data?.filter((trx) => {
        const expiryDate = new Date(trx.expiry_date);
                ...
                >
                  <div className="flex flex-col gap-2 w-full sm:w-1/2">
                    <div className="p-2 pr-3 rounded-3xl flex items-center gap-2 shadow-md bg-[#0C343C]">
                      ...
                      <p className="text-lg font-medium">{trx.plan.type}</p>
                    </div>
                  ...
                  <div className="flex max-sm:items-center flex-col gap-2 w-full sm:w-1/2 sm:text-right">
                    <div className="flex items-center gap-2 text-lg sm:justify-end justify-start">
                    ...
                    <p className="text-xs sm:text-sm text-gray-300 my-1">
                      ...
                      {new Date(trx.purchase_date).toLocaleDateString()}
                    </p>
                    <div className="btn-gradient text-black px-4 py-1 text-xs rounded-full shadow-md hover:brightness-110 transition-all self-center sm:self-end">
                      ...
                      <span className="font-semibold">
                        {new Date(trx.expiry_date).toLocaleDateString()}
                      </span>
                    </div>
                  </div>
                </div>
